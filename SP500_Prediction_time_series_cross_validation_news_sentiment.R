...
config <- list(
  ...
  output_dir = "SP_500_comprehensive_results_news_sentiment",
  checkpoint_dir = "SP_500_comprehensive_checkpoints_news_sentiment",
  ...
  base_predictors = c(
    # Same-day market returns (now safe with shifted target)
...
load_and_prepare_data <- function() {
  ...
  # CRITICAL: Create properly shifted target (predict NEXT day's return)
  data <- data %>%
    arrange(date) %>%
    mutate(
      GSPC_ret_next_day = lead(!!sym(config$target_var), 1)  # Tomorrow's return
    )
  
  cat("✓ Created next-day target variable\n")
  ...
  data_clean <- data %>%
    filter(!is.na(GSPC_ret_next_day)) %>%  # Remove last row (can't predict future)
    select(date, GSPC_ret_next_day, all_of(available_base), all_of(all_sentiment_vars)) %>%
    filter(complete.cases(.))
  
  cat("After cleaning:", nrow(data_clean), "complete observations\n")
  cat("Date range:", min(data_clean$date), "to", max(data_clean$date), "\n")
  
  # Sanity check: correlation between today's and tomorrow's returns
  same_day_corr <- cor(data_clean[[config$target_var]], data_clean$GSPC_ret_next_day, use = "complete.obs")
  cat("Sanity check - correlation between today's and tomorrow's returns:", round(same_day_corr, 4), "\n")
  
  if (abs(same_day_corr) > 0.2) {
    cat("⚠️ High correlation detected - verify target shifting is correct\n")
  } else {
    cat("✓ Low correlation confirmed - target shifting is working correctly\n")
  }
...
run_single_experiment <- function(experiment_name, train_data, test_data, feature_cols) {
  ...
  y_train <- train_data$GSPC_ret_next_day
  X_test <- test_data[, feature_cols, drop = FALSE]
  y_test <- test_data$GSPC_ret_next_day
  ...
  cat("Clean training data:", nrow(X_train), "observations\n")
  cat("Clean test data:", nrow(X_test), "observations\n")
...
<body>
    ...
    <h1>Comprehensive ML Analysis: S&P 500 Next-Day Return Prediction</h1>
    
    <div class="summary-box">
        <h2 style="color: white; border: none; margin-top: 0;">Executive Summary</h2>
        <p><strong>Objective:</strong> Predict next-day S&P 500 returns using sentiment analysis from financial TV shows, market variables, and technical indicators.</p>
        ...
        <p><strong>Key Finding:</strong> Sentiment features provide meaningful predictive power for next-day S&P 500 returns with proper target shifting eliminating data leakage concerns.</p>
    </div>
    
    <h2>Methodology</h2>
    <ul>
        <li><strong>Target Variable:</strong> Next-day S&P 500 returns (properly shifted to avoid lookahead bias)</li>
...
# =============================================================================
# ADD THESE TS CV FUNCTIONS TO BOTH S&P 500 AND VIX SCRIPTS
# =============================================================================
...
run_comprehensive_analysis <- function() {
  cat("\n", rep("=", 80), "\n")
  cat("COMPREHENSIVE ML ANALYSIS WITH TIME SERIES CV\n")
  cat(rep("=", 80), "\n\n")
  ...
  # Target column - SCRIPT SPECIFIC:
  # FOR S&P 500 SCRIPT:
  target_col <- "GSPC_ret_next_day"
...
create_results_summary <- function(all_results) {
  summary_data <- data.frame()
  
  for (experiment in names(all_results)) {
    exp_data <- all_results[[experiment]]
    
    if (!is.null(exp_data$rf$results$metrics)) {
      summary_data <- rbind(summary_data, data.frame(
        Experiment = experiment,
        Model = "Random Forest",
        RMSE = exp_data$rf$results$metrics$rmse,
        MAE = exp_data$rf$results$metrics$mae,
        R2 = exp_data$rf$results$metrics$r2,
        Correlation = exp_data$rf$results$metrics$correlation,
        Directional_Accuracy = exp_data$rf$results$metrics$directional_accuracy,
    ...
    if (!is.null(exp_data$xgb$results$metrics)) {
      summary_data <- rbind(summary_data, data.frame(
        Experiment = experiment,
        Model = "XGBoost",
        RMSE = exp_data$xgb$results$metrics$rmse,
        MAE = exp_data$xgb$results$metrics$mae,
        R2 = exp_data$xgb$results$metrics$r2,
        Correlation = exp_data$xgb$results$metrics$correlation,
        Directional_Accuracy = exp_data$xgb$results$metrics$directional_accuracy,
        Features = exp_data$n_features,
        N_Train = exp_data$n_train,
        N_Test = exp_data$n_test,
        stringsAsFactors = FALSE
...
cat("Target: Next-day S&P 500 returns with Time Series CV (no data leakage)\n\n")

# Verify all functions are loaded
required_functions <- c("load_and_prepare_data", "tune_random_forest", "tune_xgboost", 
                        "run_experiments_with_ts_cv", "create_cv_summary_table", 
                        "analyze_individual_sentiment_results")

missing_functions <- required_functions[!sapply(required_functions, exists)]
if (length(missing_functions) > 0) {
  stop("ERROR: Missing functions: ", paste(missing_functions, collapse = ", "))
}
...
# =============================================================================
# ADD THIS SECTION TO YOUR S&P 500 SCRIPT AFTER THE TIME SERIES CV COMPLETES
# This will run a final full train/test split to get feature importance
# =============================================================================
...
importance_results <- extract_best_models_and_importance(
  ...
  data, 
  "GSPC_ret_next_day"  # Your target variable
)
...
cat("Check the comprehensive_ml_report.html file for detailed results!\n")
cat("All models use proper target shifting - no data leakage issues!\n")